...
def main():
    ...
    results_dir = artifacts_root / "label_efficiency_results"
    results_dir.mkdir(parents=True, exist_ok=True)

    results_rows = []
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_csv = results_dir / f"bimgeom_label_efficiency_runs_{ts}.csv"

    for frac in fractions:
        for seed in seeds:
            tag = f"frac_{int(frac*100):02d}p_seed_{seed}"
            print(f"\n=== Running {tag} ===")
            ...
            fit_cmd = [
                ...
                "--data.class_path=bimjepa.datasets.bimgeom_datamodule_label_efficiency.BIMGEOMDataModuleLE",
            ...
            row = {"tag": tag, "fraction": frac, "seed": seed, "ckpt_path": str(ckpt_path), **metrics}
    ...
    for r in results_rows: by_frac.setdefault(r["fraction"], []).append(r)

    agg_csv = results_dir / f"bimgeom_label_efficiency_agg_{ts}.csv"
    with open(agg_csv, "w", newline="") as f:
        fieldnames = ["fraction", "n_seeds", "overall_accuracy_mean", "overall_accuracy_std",
        ...
        for frac, rows in sorted(by_frac.items()):
            def collect(k): return [r.get(k, float("nan")) for r in rows if k in r]
            w.writerow({
                "fraction": frac, "n_seeds": len(rows),
                "overall_accuracy_mean": mean(collect("overall_accuracy")), "overall_accuracy_std": std(collect("overall_accuracy")),
                "mean_class_accuracy_mean": mean(collect("mean_class_accuracy")), "mean_class_accuracy_std": std(collect("mean_class_accuracy")),
                "precision_mean": mean(collect("precision")), "precision_std": std(collect("precision")),
                "recall_mean": mean(collect("recall")), "recall_std": std(collect("recall")),
                "f1_score_mean": mean(collect("f1_score")), "f1_score_std": std(collect("f1_score")),
            })
