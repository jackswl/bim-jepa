...
METRIC_PATTERNS = {
    ...
    "precision": re.compile(r"Precision \(Macro\):\s+([0-9.]+)"),
    "recall": re.compile(r"Recall \(Macro\):\s+([0-9.]+)"),
    "f1_score": re.compile(r"F1 Score \(Macro\):\s+([0-9.]+)"),
}
...
def main():
    ...
    results_dir = artifacts_root / "n_shot_results"
    results_dir.mkdir(parents=True, exist_ok=True)

    results_rows = []
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_csv = results_dir / f"ifcnet_n_shot_runs_{ts}.csv"

    for n_shot in n_shots:
        for seed in seeds:
            tag = f"n_shot_{n_shot:02d}_seed_{seed}"
            print(f"\n=== Running {tag} ===")
            ...
            fit_cmd = [
                ...
                "--data.class_path=bimjepa.datasets.ifcnet_datamodule_n_shot.IFCNetCoreDataModuleNShot",
                f"--data.init_args.n_shot={n_shot}",
    ...
    by_n_shot: Dict[int, List[Dict[str, float]]] = {}
    for r in results_rows:
        by_n_shot.setdefault(r["n_shot"], []).append(r)

    agg_csv = results_dir / f"ifcnet_n_shot_agg_{ts}.csv"
    with open(agg_csv, "w", newline="") as f:
        ...
        for n_shot, rows in sorted(by_n_shot.items()):
            def collect(k): return [r.get(k, float("nan")) for r in rows if k in r]
            oa = collect("overall_accuracy")
            ma = collect("mean_class_accuracy")
            pr = collect("precision")
            re = collect("recall")
            f1 = collect("f1_score")
            w.writerow({
                "n_shot": n_shot,
                "n_seeds": len(rows),
                "overall_accuracy_mean": mean(oa), "overall_accuracy_std": std(oa),
                "mean_class_accuracy_mean": mean(ma), "mean_class_accuracy_std": std(ma),
                "precision_mean": mean(pr), "precision_std": std(pr),
                "recall_mean": mean(re), "recall_std": std(re),
                "f1_score_mean": mean(f1), "f1_score_std": std(f1),
            })
