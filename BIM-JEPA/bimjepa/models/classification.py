...
class BimJepaClassification(pl.LightningModule):
    def __init__(
        ...
        # metrics are instantiated in setup() once num_classes is known from the datamodule
        self.train_acc = None
        self.val_acc = None
        self.vote = vote
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.test_preds = []  # accumulated for confusion matrix at epoch end
        self.test_labels = []
        self.test_step_times = []

        if self.hparams.cls_head == "mlp":
            self.cls_head = nn.Sequential(
                # bias=False because each Linear is followed by BatchNorm1d
                nn.Linear(self.first_cls_head_dim, self.hparams.cls_head_dim * 2, bias=False),
        ...
        # encoder/tokenizer/positional_encoding start frozen; unfrozen in on_train_epoch_start
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        # points: (B, N, 3); tokens: (B, T, C); centers: (B, T, 3)
    ...
    def test_step(
        ...
        # use CUDA events (not wallclock) so timing isn't polluted by async kernel launches
    ...
    def configure_optimizers(self):
        ...
        # avoid div-by-zero when max_epochs is not set
    ...
    def load_pretrained_checkpoint(self, path: str) -> None:
        ...
        # repair NaNs in batchnorm running stats (occasionally observed in older checkpoints)
