...
# from bimjepa.datasets.ModelNet40Ply2048 import ModelNet40Ply2048DataModule
# from bimjepa.datasets.ShapeNet55 import ShapeNet55DataModule
# from bimjepa.datasets.ShapeNetPart import ShapeNetPartDataModule
...
def xy(
    ...
    idx = 0
    with torch.no_grad(): # added 23 aug
...
def main():
    #  svm_validation:
    # modelnet40:
    #   class_path: bimjepa.datasets.ModelNet40Ply2048DataModule
    #   init_args:
    ...
    # assert isinstance(cli.model, BimJepa)
    ...
    args = parse_args()
    # Call the method directly on the class (Correct)
    li_model = BimJepa.load_from_checkpoint(args.finetuned_ckpt_path, strict=False)

    li_model = li_model.cuda() # i uncommented 22 aug
    li_model.eval()
    ...
    data_module = IFCNetCoreDataModule(
        ...
        batch_size=32 # Or another batch size that fits in memory
    )
    ...
    tsne = TSNE(n_components=2, verbose=1, perplexity=50, max_iter=1000) # changed n_iter to max_iter
    ...
    # Sort the class names by their index to ensure the order is correct
    ...
    hue='Label',  
    data=df, 
     palette=sns.color_palette("hsv", 20) # 20 classes
    )
